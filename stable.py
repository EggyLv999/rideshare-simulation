...
def stable(coords,maxsize,precomp):
    num_points = len(coords[1])
    prefs = {}
    for i in xrange(num_points):
        l = sorted([j for j in xrange(num_points) if not j==i], key=lambda x: precomp[1][i][x])
        for (j,x) in enumerate(l):
            if dist(precomp,[i,x]) > precomp[0][i] + precomp[0][x]:
                l[j] = i
        prefs[i]=l

    match = stableroommate(prefs) 
    def getdist(m):
        d = 0
        for i in xrange(num_points):
            if m[i] == i:
                ...
                d += dist(precomp,[i,m[i]])/2
        return d

    if match != None:
        return getdist(match)
    ...
    for i in xrange(num_points):
        ...
        for (j,x) in enumerate(l):
            if dist(precomp,[i,x]) > precomp[0][i] + precomp[0][x]:
                l[j] = i
        prefs[i]=[num_points]+l
    prefs[num_points]=[i for i in xrange(num_points)]
